...
module PSRAM_WRITER (
    ...
    wire[7:0]        FINAL_COUNT = (mode == 0) ? 7 : (7 + size*2);

    reg         state, nstate;
    reg [7:0]   counter;
    reg [23:0]  saddr;
    wire [3:0]  init_dout, qpi_dout;
    //reg [7:0]   data [3:0];
    ...
    assign init_dout=   (counter < 8)   ?   {3'b0, cmd[7 - counter]}    : 4'h0;
    assign qpi_dout =   (counter == 0)  ?   cmd[7:4]            :
                        (counter == 1)  ?   cmd[3:0]            :
                        (counter == 2)  ?   saddr[23:20]        :
                        (counter == 3)  ?   saddr[19:16]        :
                        (counter == 4)  ?   saddr[15:12]        :
    ...
    assign dout = (mode == 0) ? init_dout : qpi_dout;
    assign douten   = (~ce_n);
